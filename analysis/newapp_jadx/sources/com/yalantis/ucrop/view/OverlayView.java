...
import com.yalantis.ucrop.C4161R;
...
public class OverlayView extends View {
    ...
    private int getCurrentTouchIndex(float f2, float f3) {
        ...
        for (int i3 = 0; i3 < 8; i3 += 2) {
            double sqrt = Math.sqrt(Math.pow(f3 - this.mCropGridCorners[i3 + 1], 2.0d) + Math.pow(f2 - this.mCropGridCorners[i3], 2.0d));
            if (sqrt < d2) {
                i2 = i3 / 2;
                d2 = sqrt;
            }
    ...
    private void initCropFrameStyle(@NonNull TypedArray typedArray) {
        int dimensionPixelSize = typedArray.getDimensionPixelSize(C4161R.styleable.ucrop_UCropView_ucrop_frame_stroke_size, getResources().getDimensionPixelSize(C4161R.dimen.ucrop_default_crop_frame_stoke_width));
        int color = typedArray.getColor(C4161R.styleable.ucrop_UCropView_ucrop_frame_color, getResources().getColor(C4161R.color.ucrop_color_default_crop_frame));
    ...
    private void initCropGridStyle(@NonNull TypedArray typedArray) {
        int dimensionPixelSize = typedArray.getDimensionPixelSize(C4161R.styleable.ucrop_UCropView_ucrop_grid_stroke_size, getResources().getDimensionPixelSize(C4161R.dimen.ucrop_default_crop_grid_stoke_width));
        int color = typedArray.getColor(C4161R.styleable.ucrop_UCropView_ucrop_grid_color, getResources().getColor(C4161R.color.ucrop_color_default_crop_grid));
        this.mCropGridPaint.setStrokeWidth(dimensionPixelSize);
        this.mCropGridPaint.setColor(color);
        this.mCropGridRowCount = typedArray.getInt(C4161R.styleable.ucrop_UCropView_ucrop_grid_row_count, 2);
        this.mCropGridColumnCount = typedArray.getInt(C4161R.styleable.ucrop_UCropView_ucrop_grid_column_count, 2);
    }
    ...
    private void smoothToCenter() {
        ...
        StringBuilder m586H = C1499a.m586H("pre");
        m586H.append(this.mCropViewRect);
        m586H.toString();
        RectF rectF2 = new RectF(this.mCropViewRect);
        rectF2.offset(centerX, centerY);
        String str = "after" + rectF2;
        ...
        this.smoothAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: com.yalantis.ucrop.view.OverlayView.1
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator2) {
                ...
                RectF rectF4 = rectF;
                rectF3.set(new RectF(rectF4.left + floatValue, rectF4.top + floatValue2, rectF4.right + floatValue, rectF4.bottom + floatValue2));
        ...
        this.smoothAnimator.addListener(new Animator.AnimatorListener() { // from class: com.yalantis.ucrop.view.OverlayView.2
        ...
        if (this.mShowCropGrid) {
            if (this.mGridPoints == null && !this.mCropViewRect.isEmpty()) {
                ...
                for (int i3 = 0; i3 < this.mCropGridRowCount; i3++) {
                    ...
                    int i5 = i4 + 1;
                    float f2 = i3 + 1.0f;
                    float height = (f2 / (this.mCropGridRowCount + 1)) * rectF.height();
                    RectF rectF2 = this.mCropViewRect;
                    ...
                    i2 = i6 + 1;
                    fArr2[i6] = ((f2 / (this.mCropGridRowCount + 1)) * rectF2.height()) + this.mCropViewRect.top;
                }
                for (int i7 = 0; i7 < this.mCropGridColumnCount; i7++) {
                    float[] fArr3 = this.mGridPoints;
                    int i8 = i2 + 1;
                    float f3 = i7 + 1.0f;
                    float width = (f3 / (this.mCropGridColumnCount + 1)) * this.mCropViewRect.width();
                    ...
                    fArr4[i8] = rectF3.top;
                    int i10 = i9 + 1;
                    float width2 = (f3 / (this.mCropGridColumnCount + 1)) * rectF3.width();
                    RectF rectF4 = this.mCropViewRect;
    ...
    public void processStyledAttributes(@NonNull TypedArray typedArray) {
        this.mCircleDimmedLayer = typedArray.getBoolean(C4161R.styleable.ucrop_UCropView_ucrop_circle_dimmed_layer, false);
        this.mDimmedColor = typedArray.getColor(C4161R.styleable.ucrop_UCropView_ucrop_dimmed_color, getResources().getColor(C4161R.color.ucrop_color_default_dimmed));
        ...
        this.mShowCropFrame = typedArray.getBoolean(C4161R.styleable.ucrop_UCropView_ucrop_show_frame, true);
        initCropGridStyle(typedArray);
        this.mShowCropGrid = typedArray.getBoolean(C4161R.styleable.ucrop_UCropView_ucrop_show_grid, true);
    }
    ...
    public OverlayView(Context context, AttributeSet attributeSet, int i2) {
        ...
        this.mTouchPointThreshold = getResources().getDimensionPixelSize(C4161R.dimen.ucrop_default_crop_rect_corner_touch_threshold);
        this.mCropRectMinSize = getResources().getDimensionPixelSize(C4161R.dimen.ucrop_default_crop_rect_min_size);
        this.mCropRectCornerTouchAreaLineLength = getResources().getDimensionPixelSize(C4161R.dimen.ucrop_default_crop_rect_corner_touch_area_line_length);
        init();
    }
}
