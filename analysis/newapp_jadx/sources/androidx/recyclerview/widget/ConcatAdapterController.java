...
public class ConcatAdapterController implements NestedAdapterWrapper.Callback {
    ...
    private WrapperAndLocalPosition findWrapperAndLocalPosition(int i2) {
        ...
        throw new IllegalArgumentException(C1499a.m626l("Cannot find wrapper for ", i2));
    }
    ...
    public int getLocalAdapterPosition(RecyclerView.Adapter<? extends RecyclerView.ViewHolder> adapter, RecyclerView.ViewHolder viewHolder, int i2) {
        ...
        StringBuilder m589K = C1499a.m589K("Detected inconsistent adapter updates. The local position of the view holder maps to ", countItemsBefore, " which is out of bounds for the adapter with size ", itemCount, ".Make sure to immediately call notify methods in your adapter when you change the backing dataviewHolder:");
        m589K.append(viewHolder);
        m589K.append("adapter:");
        m589K.append(adapter);
        throw new IllegalStateException(m589K.toString());
    }
    ...
    public boolean addAdapter(int i2, RecyclerView.Adapter<RecyclerView.ViewHolder> adapter) {
        if (i2 < 0 || i2 > this.mWrappers.size()) {
            StringBuilder m586H = C1499a.m586H("Index must be between 0 and ");
            m586H.append(this.mWrappers.size());
            m586H.append(". Given:");
            m586H.append(i2);
            throw new IndexOutOfBoundsException(m586H.toString());
        }
        if (hasStableIds()) {
