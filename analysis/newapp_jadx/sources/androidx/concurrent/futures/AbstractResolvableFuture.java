...
public abstract class AbstractResolvableFuture<V> implements InterfaceFutureC2413a<V> {
    ...
    public static final class SetFuture<V> implements Runnable {
        public final InterfaceFutureC2413a<? extends V> future;
        public final AbstractResolvableFuture<V> owner;

        public SetFuture(AbstractResolvableFuture<V> abstractResolvableFuture, InterfaceFutureC2413a<? extends V> interfaceFutureC2413a) {
            this.owner = abstractResolvableFuture;
            this.future = interfaceFutureC2413a;
        }
    ...
    public static Object getFutureValue(InterfaceFutureC2413a<?> interfaceFutureC2413a) {
        if (interfaceFutureC2413a instanceof AbstractResolvableFuture) {
            Object obj = ((AbstractResolvableFuture) interfaceFutureC2413a).value;
            if (!(obj instanceof Cancellation)) {
                return obj;
            }
        ...
        boolean isCancelled = interfaceFutureC2413a.isCancelled();
        if ((!GENERATE_CANCELLATION_CAUSES) && isCancelled) {
            return Cancellation.CAUSELESS_CANCELLED;
        }
        try {
            Object uninterruptibly = getUninterruptibly(interfaceFutureC2413a);
            return uninterruptibly == null ? NULL : uninterruptibly;
        } catch (CancellationException e2) {
            if (isCancelled) {
                return new Cancellation(false, e2);
            }
            return new Failure(new IllegalArgumentException("get() threw CancellationException, despite reporting isCancelled() == false: " + interfaceFutureC2413a, e2));
        } catch (ExecutionException e3) {
    ...
    public final boolean cancel(boolean z) {
        ...
        while (true) {
            if (ATOMIC_HELPER.casValue(abstractResolvableFuture, obj, cancellation)) {
                ...
                InterfaceFutureC2413a<? extends V> interfaceFutureC2413a = ((SetFuture) obj).future;
                if (!(interfaceFutureC2413a instanceof AbstractResolvableFuture)) {
                    interfaceFutureC2413a.cancel(z);
                    return true;
                }
                abstractResolvableFuture = (AbstractResolvableFuture) interfaceFutureC2413a;
    ...
    public final V get(long j2, TimeUnit timeUnit) {
        ...
        if (nanos + SPIN_THRESHOLD_NANOS < 0) {
            String m637w = C1499a.m637w(str, " (plus ");
            long j3 = -nanos;
            long convert = timeUnit.convert(j3, TimeUnit.NANOSECONDS);
            long nanos2 = j3 - timeUnit.toNanos(convert);
            boolean z = convert == 0 || nanos2 > SPIN_THRESHOLD_NANOS;
            if (convert > 0) {
                String str2 = m637w + convert + " " + lowerCase;
                if (z) {
                    str2 = C1499a.m637w(str2, ChineseToPinyinResource.Field.COMMA);
                }
                m637w = C1499a.m637w(str2, " ");
            }
            if (z) {
                m637w = C1499a.m631q(m637w, nanos2, " nanoseconds ");
            }
            str = C1499a.m637w(m637w, "delay)");
        }
        if (isDone()) {
            throw new TimeoutException(C1499a.m637w(str, " but future completed as timeout expired"));
        }
        throw new TimeoutException(C1499a.m639y(str, " for ", abstractResolvableFuture));
    }
    ...
    public String pendingToString() {
        Object obj = this.value;
        if (obj instanceof SetFuture) {
            return C1499a.m582D(C1499a.m586H("setFuture=["), userObjectToString(((SetFuture) obj).future), "]");
        }
        if (!(this instanceof ScheduledFuture)) {
            return null;
        }
        StringBuilder m586H = C1499a.m586H("remaining delay=[");
        m586H.append(((ScheduledFuture) this).getDelay(TimeUnit.MILLISECONDS));
        m586H.append(" ms]");
    ...
    public boolean setFuture(InterfaceFutureC2413a<? extends V> interfaceFutureC2413a) {
        Failure failure;
        checkNotNull(interfaceFutureC2413a);
        Object obj = this.value;
        if (obj == null) {
            if (interfaceFutureC2413a.isDone()) {
                if (!ATOMIC_HELPER.casValue(this, null, getFutureValue(interfaceFutureC2413a))) {
                    return false;
                }
                complete(this);
                return true;
            }
            SetFuture setFuture = new SetFuture(this, interfaceFutureC2413a);
            if (ATOMIC_HELPER.casValue(this, null, setFuture)) {
                try {
                    interfaceFutureC2413a.addListener(setFuture, DirectExecutor.INSTANCE);
                } catch (Throwable th) {
                    try {
        ...
        if (obj instanceof Cancellation) {
            interfaceFutureC2413a.cancel(((Cancellation) obj).wasInterrupted);
        }
        return false;
    }
    ...
    public String toString() {
        ...
        if (isCancelled()) {
            ...
            try {
                ...
                StringBuilder m586H = C1499a.m586H("Exception thrown from implementation: ");
                m586H.append(e2.getClass());
                sb = m586H.toString();
            }
            if (sb != null && !sb.isEmpty()) {
                C1499a.m606a0(sb2, "PENDING, info=[", sb, "]");
            } else if (isDone()) {
                addDoneString(sb2);
            } else {
                sb2.append("PENDING");
