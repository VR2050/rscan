...
public class DiagnosticsWorker extends Worker {
    ...
    private static String workSpecRow(@NonNull WorkSpec workSpec, @Nullable String str, @Nullable Integer num, @NonNull String str2) {
        return String.format("\n%s\t %s\t %s\t %s\t %s\t %s\t", workSpec.f210id, workSpec.workerClassName, num, workSpec.state.name(), str, str2);
    }
    ...
    private static String workSpecRows(@NonNull WorkNameDao workNameDao, @NonNull WorkTagDao workTagDao, @NonNull SystemIdInfoDao systemIdInfoDao, @NonNull List<WorkSpec> list) {
        ...
        for (WorkSpec workSpec : list) {
            Integer num = null;
            SystemIdInfo systemIdInfo = systemIdInfoDao.getSystemIdInfo(workSpec.f210id);
            if (systemIdInfo != null) {
                num = Integer.valueOf(systemIdInfo.systemId);
            }
            sb.append(workSpecRow(workSpec, TextUtils.join(ChineseToPinyinResource.Field.COMMA, workNameDao.getNamesForWorkSpecId(workSpec.f210id)), num, TextUtils.join(ChineseToPinyinResource.Field.COMMA, workTagDao.getTagsForWorkSpecId(workSpec.f210id))));
        }
        return sb.toString();
    }
