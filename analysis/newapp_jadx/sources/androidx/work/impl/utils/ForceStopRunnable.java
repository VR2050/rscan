...
public class ForceStopRunnable implements Runnable {
        ...
        try {
            ...
            if (z) {
                for (WorkSpec workSpec : runningWork) {
                    workSpecDao.setState(WorkInfo.State.ENQUEUED, workSpec.f210id);
                    workSpecDao.markWorkSpecScheduled(workSpec.f210id, -1L);
                }
            }
    ...
    public boolean isForceStopped() {
        if (getPendingIntent(this.mContext, Label.FORWARD_REFERENCE_TYPE_WIDE) != null) {
            return false;
        }
