...
public class PathInterpolatorCompat implements Interpolator {
    ...
    /* renamed from: mX */
    private float[] f207mX;

    /* renamed from: mY */
    private float[] f208mY;
    ...
    private void initPath(Path path) {
        ...
        this.f207mX = new float[min];
        this.f208mY = new float[min];
        float[] fArr = new float[2];
        for (int i3 = 0; i3 < min; i3++) {
            pathMeasure.getPosTan((i3 * length) / (min - 1), fArr, null);
            this.f207mX[i3] = fArr[0];
            this.f208mY[i3] = fArr[1];
        }
        if (Math.abs(this.f207mX[0]) <= 1.0E-5d && Math.abs(this.f208mY[0]) <= 1.0E-5d) {
            int i4 = min - 1;
            if (Math.abs(this.f207mX[i4] - 1.0f) <= 1.0E-5d && Math.abs(this.f208mY[i4] - 1.0f) <= 1.0E-5d) {
                float f2 = 0.0f;
                int i5 = 0;
                while (i2 < min) {
                    float[] fArr2 = this.f207mX;
                    int i6 = i5 + 1;
                    float f3 = fArr2[i5];
                    if (f3 < f2) {
        ...
        StringBuilder m586H = C1499a.m586H("The Path must start at (0,0) and end at (1,1) start: ");
        m586H.append(this.f207mX[0]);
        m586H.append(ChineseToPinyinResource.Field.COMMA);
        m586H.append(this.f208mY[0]);
        m586H.append(" end:");
        int i7 = min - 1;
        m586H.append(this.f207mX[i7]);
        m586H.append(ChineseToPinyinResource.Field.COMMA);
        m586H.append(this.f208mY[i7]);
    ...
    private void parseInterpolatorFromTypeArray(TypedArray typedArray, XmlPullParser xmlPullParser) {
        if (TypedArrayUtils.hasAttribute(xmlPullParser, "pathData")) {
            ...
            if (createPathFromPathData == null) {
                throw new InflateException(C1499a.m637w("The path is null, which is created from ", namedString));
            }
    ...
    public float getInterpolation(float f2) {
        ...
        int i2 = 0;
        int length = this.f207mX.length - 1;
        while (length - i2 > 1) {
            int i3 = (i2 + length) / 2;
            if (f2 < this.f207mX[i3]) {
                length = i3;
            } else {
                i2 = i3;
            }
        }
        float[] fArr = this.f207mX;
        float f3 = fArr[length] - fArr[i2];
        if (f3 == 0.0f) {
            return this.f208mY[i2];
        }
        float f4 = (f2 - fArr[i2]) / f3;
        float[] fArr2 = this.f208mY;
        float f5 = fArr2[i2];
