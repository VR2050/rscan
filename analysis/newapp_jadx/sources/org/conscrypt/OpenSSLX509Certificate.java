...
public final class OpenSSLX509Certificate extends X509Certificate {
    ...
    public void checkValidity(Date date) {
        if (getNotBefore().compareTo(date) > 0) {
            StringBuilder m586H = C1499a.m586H("Certificate not valid until ");
            m586H.append(getNotBefore().toString());
            m586H.append(" (compared to ");
            m586H.append(date.toString());
            m586H.append(ChineseToPinyinResource.Field.RIGHT_BRACKET);
            throw new CertificateNotYetValidException(m586H.toString());
        }
        if (getNotAfter().compareTo(date) >= 0) {
            return;
        }
        StringBuilder m586H2 = C1499a.m586H("Certificate expired at ");
        m586H2.append(getNotAfter().toString());
        m586H2.append(" (compared to ");
        m586H2.append(date.toString());
        m586H2.append(ChineseToPinyinResource.Field.RIGHT_BRACKET);
        throw new CertificateExpiredException(m586H2.toString());
    }
