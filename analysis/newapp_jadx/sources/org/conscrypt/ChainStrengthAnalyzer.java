    ...
    public static final void check(X509Certificate[] x509CertificateArr) {
        for (X509Certificate x509Certificate : x509CertificateArr) {
            try {
                ...
                throw new CertificateException(m586H.toString(), e2);
            }
        }
    }
    ...
    private static void checkKeyLength(X509Certificate x509Certificate) {
        ...
        if (publicKey instanceof ECPublicKey) {
            ...
            if (!(publicKey instanceof DSAPublicKey)) {
                StringBuilder m586H = C1499a.m586H("Rejecting unknown key class ");
                m586H.append(publicKey.getClass().getName());
                throw new CertificateException(m586H.toString());
            }
    ...
    private static void checkSignatureAlgorithm(X509Certificate x509Certificate) {
        ...
        for (String str : SIGNATURE_ALGORITHM_OID_BLACKLIST) {
            if (sigAlgOID.equals(str)) {
                throw new CertificateException(C1499a.m637w("Signature uses an insecure hash function: ", sigAlgOID));
            }
        }
    }
