// src/cores/netscan_en/arp_scan.rs
...
impl ArpScanner {
    ...
    pub async fn scan_cidr(&self, cidr: &str) -> Result<Vec<ArpHost>, RustpenError> {
        // 解析 CIDR
        let net: Ipv4Network = cidr.parse::<Ipv4Network>().map_err(|e| RustpenError::ParseError(e.to_string()))?;
        ...
        let hosts = tokio::task::spawn_blocking(move || {
            Self::scan_on_interface(&cfg, &targets)
        })
        .await
    ...
    fn choose_interface<'a>(maybe_name: &Option<String>) -> Result<NetworkInterface, RustpenError> {
        ...
        interfaces.into_iter()
            .find(|i| {
                !i.is_loopback() && i.mac.is_some() && i.ips.iter().any(|ip| ip.is_ipv4())
            })
            .ok_or_else(|| RustpenError::ScanError("没有可用的网络接口用于 ARP 扫描".to_string()))
    }

    fn scan_on_interface(cfg: &ArpConfig, targets: &[Ipv4Addr]) -> Result<Vec<ArpHost>, RustpenError> {
        let iface = Self::choose_interface(&cfg.interface)?;
        let src_mac = iface.mac.ok_or_else(|| RustpenError::ScanError("接口没有 MAC 地址".to_string()))?;
        let src_ip = iface.ips.iter().find_map(|ip| if ip.is_ipv4() { Some(match ip.ip() { std::net::IpAddr::V4(v4) => v4, _ => unreachable!() }) } else { None }).ok_or_else(|| RustpenError::ScanError("接口没有 IPv4 地址".to_string()))?;

        // 打开 datalink 通道
        let (mut tx, mut rx) = match datalink::channel(&iface, Default::default()) {
            Ok(Ethernet(tx, rx)) => (tx, rx),
            Ok(_) => return Err(RustpenError::ScanError("未能打开以太网通道(非以太网)".to_string())),
                        ...
                        if eth.get_ethertype() == EtherTypes::Arp {
                            if let Some(arp) = ArpPacket::new(eth.payload()) {
                                if arp.get_operation() == ArpOperations::Reply {
                                    ...
                                    if unique_targets.contains(&sender_ip) || true {
                                        // 记录
        ...
        // 构造结果
        let mut res: Vec<ArpHost> = found.into_iter().map(|(ip, mac)| ArpHost { ip, mac, interface: iface.name.clone() }).collect();
        // 排序以保证稳定性
