// src/cores/netscan_en/syn_scan.rs
...
impl SynScanner {
    ...
    async fn check_single_port(&self, host: IpAddr, port: u16) -> PortResult {
        ...
        let pnet_res = match host {
            IpAddr::V4(dest_v4) => {
                ...
                tokio::task::spawn_blocking(move || -> Result<PortStatus, String> {
                    ...
                    use pnet::transport::{transport_channel, TransportChannelType, tcp_packet_iter};
                    use pnet::transport::TransportProtocol::Ipv4 as TPIpv4;
                    use std::time::Duration as StdDuration;

                    let local_ip = get_local_ipv4_for_destination(dest_v4).map_err(|e| format!("获取本地IP失败: {}", e))?;
                    ...
                    let source_port: u16 = if src_port != 0 { src_port } else { 40000 + (port % 1000) };
                    ...
                    let checksum = pnet::packet::tcp::ipv4_checksum(&tcp_packet.to_immutable(), &local_ip, &dest_v4);
                    ...
                    let until = std::time::Instant::now() + StdDuration::from_secs(timeout_dur.as_secs());
                    while std::time::Instant::now() < until {
                        match iter.next_with_timeout(StdDuration::from_millis(250)) {
                            Ok(Some((packet, ip))) => {
                                if ip == IpAddr::V4(dest_v4) && packet.get_destination() == source_port && packet.get_source() == port {
            ...
            IpAddr::V6(dest_v6) => {
                ...
                tokio::task::spawn_blocking(move || -> Result<PortStatus, String> {
                    ...
                    let local_v6 = get_local_ipv6_for_destination(dest_v6).map_err(|e| format!("获取本地IPv6失败: {}", e))?;
                    ...
                    // IPv6 校验和
                    let checksum = pnet::packet::tcp::ipv6_checksum(&tcp_packet.to_immutable(), &local_v6, &dest_v6);
                    tcp_packet.set_checksum(checksum);
                    ...
                    while std::time::Instant::now() < until {
                        match iter.next_with_timeout(StdDuration::from_millis(250)) {
                            Ok(Some((packet, ip))) => {
                                if ip == IpAddr::V6(dest_v6) && packet.get_destination() == source_port && packet.get_source() == port {
                                    let flags = packet.get_flags();
                                    if flags & (TcpFlags::SYN | TcpFlags::ACK) == (TcpFlags::SYN | TcpFlags::ACK) {
                                        return Ok(PortStatus::Open);
                                    }
        ...
        match timeout(Duration::from_secs(self.config.timeout_seconds), tokio::net::TcpStream::connect(&addr)).await {
            Ok(Ok(_stream)) => {
                let latency = start_time.elapsed().as_millis() as u16;
                PortResult::new(port, PortStatus::Open, Protocol::Tcp).with_latency(latency)
            }
            Ok(Err(e)) => {
                let status = if e.kind() == std::io::ErrorKind::ConnectionRefused { PortStatus::Closed } else { PortStatus::Filtered };
...
fn get_local_ipv4_for_destination(dest: Ipv4Addr) -> Result<Ipv4Addr, std::io::Error> {
    ...
    Err(std::io::Error::new(std::io::ErrorKind::Other, "无法获取本地IPv4"))
}

/// 获取本地 IPv6 地址，方法与 IPv4 相同但使用 IPv6 未指定地址
fn get_local_ipv6_for_destination(dest: std::net::Ipv6Addr) -> Result<std::net::Ipv6Addr, std::io::Error> {
    ...
    Err(std::io::Error::new(std::io::ErrorKind::Other, "无法获取本地IPv6"))
}
...
mod tests {
    ...
    async fn concurrent_syn_scan_returns_open_ports() {
        ...
        let config = SynConfig { timeout_seconds: 2, concurrent: true, concurrency: 4, source_port: 0 };
        let scanner = SynScanner::new(config);

        // 如果 pnet 无法在当前环境使用，会回退到 TCP connect，测试仍然有效
        let res = scanner.scan_ports("127.0.0.1".parse().unwrap(), &[addr.port()]).await.unwrap();
    ...
    async fn concurrent_syn_scan_ipv6_returns_open_ports() {
        ...
        // IPv6 扫描目标为 ::1
        let res = scanner.scan_ports("::1".parse().unwrap(), &[addr.port()]).await.unwrap();
        assert!(res.open_ports_count() >= 1);
    }
