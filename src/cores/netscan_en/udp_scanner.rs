// src/cores/netscan_en/scanner/udp_scanner.rs
...
impl UdpScanner {
    ...
    /// 复用 socket 的顺序扫描实现
    async fn scan_ports_sequential_with_socket(&self, host: IpAddr, ports: &[u16], socket: &UdpSocket) -> ScanResult {
        let start_time = Instant::now();

        let mut scan_result = ScanResult::new(host.to_string(), host, Protocol::Udp);

        // 顺序扫描每个端口，复用同一个 socket
        for &port in ports {
            let port_result = self.check_single_port_with_socket(host, port, socket).await;
    ...
    /// 创建绑定到随机端口的UDP socket
    async fn create_udp_socket(&self) -> Result<UdpSocket, RustpenError> {
        // Deprecated: prefer create_udp_socket_for_host which considers IPv6
        UdpSocket::bind("0.0.0.0:0").await.map_err(|e| RustpenError::Io(e))
    }
    ...
    async fn create_udp_socket_for_host(&self, host: IpAddr) -> Result<UdpSocket, RustpenError> {
        ...
        let socket = UdpSocket::bind(bind_addr).await.map_err(|e| RustpenError::Io(e))?;
    ...
    async fn check_single_port(&self, host: IpAddr, port: u16) -> PortResult {
        ...
        match self.create_udp_socket_for_host(host).await {
            Ok(socket) => self.check_single_port_with_socket(host, port, &socket).await,
            Err(_) => PortResult::new(port, PortStatus::Error, Protocol::Udp),
        }
    }

    /// 使用指定的 socket 执行一次端口检查（支持重试）
    async fn check_single_port_with_socket(&self, host: IpAddr, port: u16, socket: &UdpSocket) -> PortResult {
        let start_time = Instant::now();
        ...
        loop {
            ...
            match recv_res {
                Ok(Ok((size, _))) => {
                    ...
                    let mut result = PortResult::new(port, PortStatus::Open, Protocol::Udp).with_latency(latency);
                    if let Some(b) = banner { result = result.with_banner(b); }
                    return result;
                }
                Ok(Err(_)) => return PortResult::new(port, PortStatus::Error, Protocol::Udp),
                Err(_) => {
    ...
    async fn scan_ports_concurrent(&self, host: IpAddr, ports: &[u16]) -> ScanResult {
        ...
        // 复用单个socket并克隆给并发任务
        let socket = match self.create_udp_socket_for_host(host).await {
            Ok(s) => s,
            Err(_) => return scan_result,
        };
        ...
        let results: Vec<PortResult> = stream::iter(ports.iter().copied())
            .map(|port| {
                ...
                async move { scanner.check_single_port_with_socket(host, port, &socket).await }
            })
...
mod tests {
    use super::*;
    use crate::cores::netscan_en::tcp_scanner::PortScanner;

    #[tokio::test]
    async fn udp_scan_detects_open_port_and_reuses_socket() {
        // 创建 UDP 服务端并回显
        ...
        let res = scanner.scan_ports("127.0.0.1".parse().unwrap(), &[port]).await.unwrap();
    ...
    fn test_udp_scanner(){
        ...
        println!("测试UDP端口扫描，目标主机：{}，端口：{}",target,ports);
         ...
         let scan_result=rt.block_on(scanner.scan_ports(target.parse().unwrap(),&port_vec)).unwrap();
