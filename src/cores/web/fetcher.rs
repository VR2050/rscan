...
impl Fetcher {
    ...
    pub async fn fetch_with_request(
        &self,
        req: FetchRequest,
    ) -> Result<FetchResponse, RustpenError> {
        // 按 host 限流
        let url = req.url.clone();
        let parsed =
            url::Url::parse(&url).map_err(|e| RustpenError::NetworkError(e.to_string()))?;
        let host = parsed
            .host_str()
            .ok_or_else(|| RustpenError::NetworkError("no host".to_string()))?
            .to_string();
                ...
                match tokio::time::timeout(dur, send_fut).await {
                    ...
                    Err(_) => {
                        ...
                        if attempts > max_retries {
                            return Err(RustpenError::TargetUnreachable {
                                url: req.url.clone(),
                            });
                        }
...
mod tests {
    ...
    async fn fetcher_gets_body_and_headers() {
        ...
        assert!(
            resp.headers
                .iter()
                .any(|(k, v)| k.to_lowercase() == "x-test" && v == "1")
        );
