// src/cores/host/scanner/manager.rs
use std::net::IpAddr;
...
impl ScanManager {
    ...
    /// 扫描单个UDP端口（如果已启用）
    pub async fn udp_scan_port(&self, host: &str, port: u16) -> Result<ScanResult, RustpenError> {
        let udp_scanner = self
            .udp_scanner
            .as_ref()
            .ok_or_else(|| RustpenError::ScanError("UDP扫描未启用".to_string()))?;
    ...
    /// 扫描单个 SYN 端口（如果已启用）
    pub async fn syn_scan_port(&self, host: &str, port: u16) -> Result<ScanResult, RustpenError> {
        let syn_scanner = self
            .syn_scanner
            .as_ref()
            .ok_or_else(|| RustpenError::ScanError("SYN扫描未启用".to_string()))?;
    ...
    pub async fn scan_both_protocols(
        ...
        if let Some(udp_scanner) = &self.udp_scanner {
            let (tcp_result, udp_result) =
                tokio::try_join!(tcp_future, udp_scanner.scan_ports(ip, udp_ports))?;
    ...
    /// 快速扫描所有协议
    pub async fn quick_scan_all(
        &self,
        host: &str,
    ) -> Result<(Option<ScanResult>, Option<ScanResult>), RustpenError> {
        use super::ports::common_ports;
    ...
    pub async fn scan_port(
        ...
        match protocol {
            ...
            Protocol::Dns => self.udp_scan_port(host, port).await,
            Protocol::Icmp => Err(RustpenError::ScanError(
                "ICMP单端口扫描不适用scan_port接口".to_string(),
            )),
            Protocol::Arp => Err(RustpenError::ScanError(
                "ARP扫描不适用scan_port接口".to_string(),
            )),
        }
    }
...
mod tests {
    use super::*;
    use crate::cores::host::models::Protocol;
    ...
    async fn scan_both_protocols_udp_disabled_returns_tcp_result() {
        ...
        let (tcp_res, udp_res) = manager
            .scan_both_protocols("127.0.0.1", &ports, &[])
            .await
            .unwrap();
