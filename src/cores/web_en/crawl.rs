...
use crate::cores::web_en::{Fetcher, Parser};
...
impl Crawler {
    pub fn new(fetcher: Fetcher, config: CrawlConfig) -> Self {
        Self { fetcher, config, robots_cache: Arc::new(RwLock::new(HashMap::new())) }
    }

    /// 获取并解析 robots.txt（缓存结果）
    async fn fetch_robots_for(&self, url: &Url) -> Option<RobotsPolicy> {
        let host = url.host_str()?.to_string();
        // cached?
        {
            let cache = self.robots_cache.read().await;
            if let Some(pol) = cache.get(&host) {
                // 简单过期策略：1 小时
                if pol.fetched_at.elapsed() < Duration::from_secs(60 * 60) {
        ...
        let port_str = match url.port() { Some(p) => format!(":{}", p), None => "".to_string() };
        let robots_url = format!("{}://{}{}{}", scheme, host, port_str, "/robots.txt");
        match self.fetcher.fetch(&robots_url).await {
            Ok(resp) => {
                if resp.status == 200 {
                    let txt = String::from_utf8_lossy(&resp.body).to_string();
                    let mut disallow = Vec::new();
                    let mut crawl_delay = None;
                    let mut in_group = false; // whether current group applies to '*'
                    ...
                    let policy = RobotsPolicy { disallow, crawl_delay_ms: crawl_delay, fetched_at: Instant::now() };
                    let mut cache = self.robots_cache.write().await;
                    cache.insert(host.clone(), policy.clone());
                    return Some(policy);
                }
                None
            }
    ...
    pub async fn crawl(&self, seeds: Vec<String>) -> Result<Vec<String>, RustpenError> {
        ...
        for s in seeds { frontier.lock().await.push_back((s, 0)); }
        ...
        // Helper to pop a job
        let pop_job = |frontier: Arc<Mutex<VecDeque<(String, usize)>>>, visited: Arc<Mutex<HashSet<String>>>| async move {
            let mut f = frontier.lock().await;
            while let Some((u, d)) = f.pop_front() {
            ...
            let h = tokio::spawn(async move {
                loop {
                    ...
                    if let Some(max_pages) = cfg.max_pages {
                        ...
                        if fetched_len >= max_pages { break; }
                    }

                    // parse url
                    let parsed_url = match Url::parse(&url_s) {
                        Ok(u) => u,
                        Err(_) => { visited.lock().await.insert(url_s.clone()); continue; }
                    };
                    ...
                    if cfg.obey_robots {
                        if let Some(pol) = {
                            ...
                            if cfg.obey_robots {
                                ...
                                let port_str = match parsed_url.port() { Some(p) => format!(":{}", p), None => "".to_string() };
                                let robots_url = format!("{}://{}{}{}", scheme, host, port_str, "/robots.txt");
                                if let Ok(r) = fetcher.fetch(&robots_url).await {
                                    if r.status == 200 {
                                        ...
                                        for line in txt.lines() {
                                            ...
                                            let key = parts[0].trim().to_lowercase();
                                            let val = parts[1].trim();
                                            if key == "user-agent" { in_group = val == "*"; }
                                            else if key == "disallow" && in_group { disallow.push(val.to_string()); }
                                            else if key == "crawl-delay" && in_group { if let Ok(sec) = val.parse::<u64>() { crawl_delay = Some(sec * 1000); } }
                                        }
                                        let policy = RobotsPolicy { disallow, crawl_delay_ms: crawl_delay, fetched_at: Instant::now() };
                                        // cache it
                                        let mut c = robots_cache.write().await;
                                        c.insert(parsed_url.host_str().unwrap_or("").to_string(), policy.clone());
                    ...
                    let delay_ms = {
                        let cache = robots_cache.read().await;
                        cache.get(&host).and_then(|p| p.crawl_delay_ms).unwrap_or(cfg.default_host_delay_ms)
                    };
                    ...
                    match fetcher.fetch(&url_s).await {
                        Ok(resp) => {
                            ...
                            if depth < cfg.max_depth {
                                if let Ok(parsed) = Parser::parse(&url_s, std::str::from_utf8(&resp.body).unwrap_or("")) {
                                    let mut f = frontier.lock().await;
                                    let v = visited.lock().await;
...
mod tests {
    ...
    async fn crawl_obeys_robots_txt() {
        ...
        let route = warp::path::end().map(|| warp::reply::html("<a href=\"/open\">open</a><a href=\"/secret\">secret</a>"));
        ...
        let (addr, server) = warp::serve(route.or(secret).or(open).or(robots_route)).bind_ephemeral(([127,0,0,1],0));
        tokio::spawn(server);

        let base = format!("http://{}", addr);

        let cfg = Fetcher::new(crate::cores::web_en::FetcherConfig { timeout: Duration::from_secs(5), user_agent: None, max_retries: 0, accept_invalid_certs: false, per_host_concurrency: 2, default_headers: None, honor_retry_after: true, backoff_base_ms: 100, backoff_max_ms: 10_000 }).unwrap();
        let crawl_cfg = CrawlConfig { max_depth: 1, concurrency: 1, max_pages: Some(10), obey_robots: true, default_host_delay_ms: 1 };
    ...
    async fn crawl_ignores_robots_when_disabled() {
        ...
        let cfg = Fetcher::new(crate::cores::web_en::FetcherConfig { timeout: Duration::from_secs(5), user_agent: None, max_retries: 0, accept_invalid_certs: false, per_host_concurrency: 2, default_headers: None, honor_retry_after: true, backoff_base_ms: 100, backoff_max_ms: 10_000 }).unwrap();
        let crawl_cfg = CrawlConfig { max_depth: 1, concurrency: 1, max_pages: Some(10), obey_robots: false, default_host_delay_ms: 1 };
        let crawler = Crawler::new(cfg, crawl_cfg);
