use crate::cores::web::Fetcher;
use crate::cores::web::FetcherConfig as CoreFetcherConfig;
use crate::cores::web::crawl::{CrawlConfig, Crawler};
...
impl WebScanner {
    pub fn new(cfg: WebScanConfig) -> Result<Self, RustpenError> {
        let fetcher = Fetcher::new(cfg.fetcher)?;
    ...
    pub async fn dir_scan(
        &self,
        base: &str,
        paths: &[&str],
        cfg: Option<crate::modules::web_scan::ModuleScanConfig>,
    ...
    pub fn dir_scan_with_callback<F>(
        &self,
        base: &str,
        paths: Vec<String>,
        cfg: Option<crate::modules::web_scan::ModuleScanConfig>,
    ...
    where
        F: Fn(crate::modules::web_scan::ModuleScanResult) + Send + Sync + 'static,
    {
        let cfg = cfg.unwrap_or_default();
        crate::modules::web_scan::run_dir_scan_with_callback(base, paths, cfg, cb)
    }

    pub async fn fuzz_scan(
        &self,
        url_with_fuzz: &str,
        keywords: &[&str],
        cfg: Option<crate::modules::web_scan::ModuleScanConfig>,
    ...
    {
        let cfg = cfg.unwrap_or_default();
        crate::modules::web_scan::run_subdomain_burst_stream(base_domain, words, cfg)
    }
}
...
mod tests {
    ...
    async fn webscanner_basic_crawl() {
        ...
        let cfg = WebScanConfig {
            max_depth: 1,
            concurrency: 1,
            max_pages: Some(10),
            obey_robots: false,
            default_host_delay_ms: 1,
            fetcher: CoreFetcherConfig {
    ...
    async fn webscanner_respects_robots() {
        ...
        let route =
            warp::path::end().map(|| warp::reply::html("<a href=\"/x\">x</a><a href=\"/y\">y</a>"));
        ...
        let (addr, server) =
            warp::serve(route.or(x).or(y).or(robots_route)).bind_ephemeral(([127, 0, 0, 1], 0));
        tokio::spawn(server);
    ...
    async fn webscanner_fuzz_and_dns_integration() {
        ...
        let res = ws
            .fuzz_scan(&format!("{}/FUZZ.html", base), &["test", "a"], Some(mcfg))
            .await
            .unwrap();
        ...
        // dns (subdomain) test
        let handler =
            warp::any().map(|| warp::reply::with_status("ok", warp::http::StatusCode::OK));
