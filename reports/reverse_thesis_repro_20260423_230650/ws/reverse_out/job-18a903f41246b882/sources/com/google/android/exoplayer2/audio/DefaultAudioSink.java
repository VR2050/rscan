...
public final class DefaultAudioSink implements AudioSink {
    ...
    public void configure(int inputEncoding, int inputChannelCount, int inputSampleRate, int specifiedBufferSize, int[] outputChannels, int trimStartFrames, int trimEndFrames) throws AudioSink.ConfigurationException {
        ...
        if (Util.SDK_INT < 21 && channelCount == 8 && outputChannels == null) {
            ...
            for (int i = 0; i < outputChannels2.length; i++) {
                outputChannels2[i] = i;
            }
        } else {
        ...
        if (!flush && isInitialized() && this.outputEncoding == sampleRate && this.outputSampleRate == channelCount2 && this.outputChannelConfig == channelConfig) {
            return;
        }
        flush();
    ...
    @Override // com.google.android.exoplayer2.audio.AudioSink
    public boolean handleBuffer(ByteBuffer buffer, long presentationTimeUs) throws AudioSink.WriteException, AudioSink.InitializationException {
        ...
        if (this.inputBuffer == null) {
            ...
            if (!this.isInputPcm && this.framesPerEncodedSample == 0) {
                int framesPerEncodedSample = getFramesPerEncodedSample(this.outputEncoding, buffer);
