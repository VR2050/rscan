    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int a = 0; a < this.pagesCount; a++) {
            if (a != this.currentPage) {
                this.rect.set(AndroidUtilities.dp(11.0f) * a, 0.0f, AndroidUtilities.dp(5.0f) + r3, AndroidUtilities.dp(5.0f));
        ...
        int x = this.currentPage * AndroidUtilities.dp(11.0f);
        if (this.progress == 0.0f) {
            this.rect.set(x, 0.0f, AndroidUtilities.dp(5.0f) + x, AndroidUtilities.dp(5.0f));
        } else if (this.scrollPosition >= this.currentPage) {
            this.rect.set(x, 0.0f, AndroidUtilities.dp(5.0f) + x + (AndroidUtilities.dp(11.0f) * this.progress), AndroidUtilities.dp(5.0f));
        } else {
            this.rect.set(x - (AndroidUtilities.dp(11.0f) * (1.0f - this.progress)), 0.0f, AndroidUtilities.dp(5.0f) + x, AndroidUtilities.dp(5.0f));
        }
        canvas.drawRoundRect(this.rect, AndroidUtilities.dp(2.5f), AndroidUtilities.dp(2.5f), this.paint);
    }
}
