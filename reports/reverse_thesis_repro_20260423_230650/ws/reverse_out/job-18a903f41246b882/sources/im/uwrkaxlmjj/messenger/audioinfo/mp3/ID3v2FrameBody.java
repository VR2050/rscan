...
import im.uwrkaxlmjj.messenger.audioinfo.util.RangeInputStream;
import java.io.IOException;
import java.io.InputStream;

/* JADX INFO: loaded from: classes2.dex */
public class ID3v2FrameBody {
    static final ThreadLocal<Buffer> textBuffer = new ThreadLocal<Buffer>() { // from class: im.uwrkaxlmjj.messenger.audioinfo.mp3.ID3v2FrameBody.1
    ...
    public String readZeroTerminatedString(int maxLength, ID3v2Encoding encoding) throws ID3v2Exception, IOException {
        ...
        for (int i = 0; i < length; i++) {
            ...
            bytes[i] = b;
            if (b == 0 && (encoding != ID3v2Encoding.UTF_16 || zeros != 0 || i % 2 == 0)) {
                zeros++;
                if (zeros == encoding.getZeroBytes()) {
