...
public class DependencyGraph {
    ...
    private String generateDisplayNode(DependencyNode node, boolean centeredConnection, String content) {
        for (DependencyNode target : node.targets) {
            String constraint = (ShellAdbUtils.COMMAND_LINE_END + node.name()) + " -> " + target.name();
            ...
            content = content + (constraint + ShellAdbUtils.COMMAND_LINE_END);
        }
        return content;
    }
    ...
    private String generateChainDisplayGraph(ChainRun chain, String content) {
        ...
        String subgroup = "subgraph " + (orientation == 0 ? name + "_h" : name + "_v") + " {\n";
        String definitions = "";
        for (WidgetRun run : chain.widgets) {
            String runName = run.widget.getDebugName();
            subgroup = subgroup + (orientation == 0 ? runName + "_HORIZONTAL" : runName + "_VERTICAL") + ";\n";
        ...
        if ((root instanceof HorizontalWidgetRun) || ((root instanceof ChainRun) && ((ChainRun) root).orientation == 0)) {
            ...
            if (behaviour == ConstraintWidget.DimensionBehaviour.FIXED || behaviour == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT) {
                if (!start.targets.isEmpty() && end.targets.isEmpty()) {
                    String constraint = ShellAdbUtils.COMMAND_LINE_END + end.name() + " -> " + start.name() + ShellAdbUtils.COMMAND_LINE_END;
                    content3 = content3 + constraint;
                } else if (start.targets.isEmpty() && !end.targets.isEmpty()) {
                    String constraint2 = ShellAdbUtils.COMMAND_LINE_END + start.name() + " -> " + end.name() + ShellAdbUtils.COMMAND_LINE_END;
                ...
                String str = ShellAdbUtils.COMMAND_LINE_END + name + "_HORIZONTAL -> " + name + "_VERTICAL;\n";
            }
            ...
            if (behaviour2 == ConstraintWidget.DimensionBehaviour.FIXED || behaviour2 == ConstraintWidget.DimensionBehaviour.WRAP_CONTENT) {
                if (!start.targets.isEmpty() && end.targets.isEmpty()) {
                    String constraint3 = ShellAdbUtils.COMMAND_LINE_END + end.name() + " -> " + start.name() + ShellAdbUtils.COMMAND_LINE_END;
                    content3 = content3 + constraint3;
                } else if (start.targets.isEmpty() && !end.targets.isEmpty()) {
                    String constraint4 = ShellAdbUtils.COMMAND_LINE_END + start.name() + " -> " + end.name() + ShellAdbUtils.COMMAND_LINE_END;
                ...
                String str2 = ShellAdbUtils.COMMAND_LINE_END + name2 + "_VERTICAL -> " + name2 + "_HORIZONTAL;\n";
            }
        }
