...
public class PasswordEditText extends AppCompatEditText implements InputType {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        this.cursorWidth = typedArray.getDimension(5, AndroidUtilities.dp(2.0f));
        this.cursorHeight = typedArray.getDimension(4, AndroidUtilities.dp(20.0f));
        this.cursorFlashInterval = typedArray.getInteger(3, 3);
        if (1 == this.mType) {
            this.mMarginEach = typedArray.getDimension(9, AndroidUtilities.dp(5.0f));
        } else {
            this.mMarginEach = typedArray.getDimension(9, AndroidUtilities.dp(0.0f));
        }
        typedArray.recycle();
    ...
    private void setCursor(boolean onlyCursorCallback) {
        ...
        if (onlyCursorCallback) {
            if (this.cursorCallback == null) {
                this.cursorCallback = new Runnable() { // from class: im.uwrkaxlmjj.ui.hviews.PasswordEditText.1
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (f > 0.0f) {
            int i = this.maxCount;
            this.mWidthItem = ((w - (f * (i - 1))) - ((this.mLineWidth * i) * 2.0f)) / i;
        } else {
            float f2 = this.mLineWidth;
            this.mWidthItem = (w - (f2 * (r4 + 1))) / this.maxCount;
        }
        this.mRealHeight = (int) (h - (this.mLineWidth / 2.0f));
    ...
    private void drawBottomLine(Canvas canvas) {
        setPaint(this.mLineWidth, Paint.Style.FILL, this.mLineColor);
        float y = getMeasuredHeight() - (this.mLineWidth / 2.0f);
        for (int i = 0; i < this.maxCount; i++) {
    ...
    private void drawItemFocused(Canvas canvas, int position) {
        ...
        float y = getMeasuredHeight() - (this.mLineWidth / 2.0f);
        float[] centers = getCenter(position);
        float f = centers[0];
        float f2 = this.mWidthItem;
        canvas.drawLine(f - (f2 / 2.0f), y, centers[0] + (f2 / 2.0f), y, this.mPaint);
    }
    ...
    private void drawCirclePoint(Canvas canvas) {
        ...
        for (int i = 0; i < this.position; i++) {
            float[] centers = getCenter(i);
            canvas.drawCircle(centers[0], centers[1], this.mCrclePointRadius, this.mPaint);
        }
    }

    private void drawText(Canvas canvas) {
        setPaint(5.0f, Paint.Style.FILL, this.mCirclePointColor);
        String t = ((Object) getText()) + "";
        if (t.length() > 0) {
            for (int i = 0; i < this.position; i++) {
                CharSequence chars = t.substring(i, i + 1);
                float w = this.mPaint.measureText(chars, 0, 1);
                Paint.FontMetrics fm = this.mPaint.getFontMetrics();
                float h = (float) Math.ceil(fm.descent - fm.ascent);
                float[] centers = getCenter(i);
                float x = centers[0] - (w / 2.0f);
                float y = (centers[1] + (h / 2.0f)) - fm.descent;
                canvas.drawText(t.substring(i, i + 1), x, y, this.mPaint);
            }
        }
    }
    ...
    private RectF getRectFByPosition(int position) {
        float left;
        float f = this.mMarginEach;
        if (f > 0.0f) {
            float f2 = (this.mWidthItem + f) * position;
            float f3 = this.mLineWidth;
            left = f2 + (position * 2 * f3) + (f3 / 2.0f);
        } else {
            float f4 = this.mWidthItem;
            float f5 = this.mLineWidth;
            left = (f5 / 2.0f) + ((f4 + f5) * position);
        }
        RectF rectF = this.rectF;
        float f6 = this.mLineWidth;
        rectF.set(left, f6 / 2.0f, this.mWidthItem + left + (f6 / 2.0f), this.mRealHeight);
        return this.rectF;
    }

    private float[] getCenter(int position) {
        float f = this.mWidthItem;
        float[] centers = {(f / 2.0f) + (position * (f + this.mMarginEach)) + (((position * 2) + 1) * this.mLineWidth), getMeasuredHeight() / 2.0f};
        return centers;
    }
    ...
    public PasswordEditText setMaxCount(int maxCount) {
    ...
    public PasswordEditText setFocusedColor(int mFocusedColor) {
        this.mFocusedColor = mFocusedColor;
        invalidate();
        return this;
    }

    public PasswordEditText setShowCursor(boolean showCursor) {
        this.showCursor = showCursor;
        invalidate();
        return this;
    }

    public PasswordEditText setCursorColor(int cursorColor) {
    ...
    public PasswordEditText setCursorWidth(float cursorWidth) {
    ...
    public PasswordEditText setCursorHeight(float cursorHeight) {
    ...
    public PasswordEditText setCursorFlashInterval(int cursorFlashInterval) {
        this.cursorFlashInterval = cursorFlashInterval;
        setCursor(true);
        invalidate();
        return this;
    }
    ...
    public static abstract class OnPasswordListenerSimple implements OnPasswordListener {
        @Override // im.uwrkaxlmjj.ui.hviews.PasswordEditText.OnPasswordListener
