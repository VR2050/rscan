...
public class DownloadUtils {
    ...
    private final Runnable mQueryProgressRunnable = new Runnable() { // from class: im.uwrkaxlmjj.ui.utils.DownloadUtils.1
    ...
    private void registerReceiver() {
        this.mDownloadReceiver = new BroadcastReceiver() { // from class: im.uwrkaxlmjj.ui.utils.DownloadUtils.2
    ...
    public void queryProgress() {
        ...
        if (this.mIsDownloading) {
            Cursor c = this.mDownloadManager.query(new DownloadManager.Query().setFilterById(this.mDownloadId));
            if (c != null && c.moveToFirst()) {
                ...
                if (status == 2) {
                    ...
                    if (totalSize > 0) {
                        float percent = (soFarSize * 1.0f) / totalSize;
                        logDebug(String.format("total:%s kb soFar:%s kb", Long.valueOf(totalSize / 1024), Long.valueOf(soFarSize / 1024)) + percent);
                        DownloadListener downloadListener2 = this.mDownLoadListener;
                        if (downloadListener2 != null) {
                            downloadListener2.onProgress((int) (100.0f * percent), soFarSize / 1024, totalSize / 1024);
                        }
                    }
                } else if (status == 8) {
                    stopQueryProgress();
