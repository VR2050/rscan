...
public class MyScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
    ...
    */
    public boolean onInterceptTouchEvent(android.view.MotionEvent r18) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: im.uwrkaxlmjj.ui.hviews.MyScrollView.onInterceptTouchEvent(android.view.MotionEvent):boolean");
    }
            ...
            if (actionMasked == 2) {
                ...
                if (activePointerIndex == -1) {
                    ...
                    if (this.mIsBeingDragged) {
                        ...
                        if (oldY <= 0 && deltaY2 < 0) {
                            float scrollRate = getScrollRate(oldY, this.mOverScrollDistance);
                            deltaY2 = (int) (deltaY2 * scrollRate);
                        }
                        if (oldY >= range && deltaY2 > 0) {
                            deltaY = (int) (deltaY2 * getScrollRate(oldY - range, this.mOverScrollDistance));
                        } else {
                            deltaY = deltaY2;
                        }
                        z = true;
    ...
    private float getScrollRate(int scrollY, int totalDistance) {
        float q = 1.0f - this.interpolator.getInterpolation((Math.abs(scrollY) * 1.0f) / Math.abs(totalDistance));
        return INIT_RATE * q;
    }
    ...
    public void computeScroll() {
        ...
        if (this.mScroller.computeScrollOffset()) {
            ...
            if (dy != 0) {
                ...
                dispatchNestedScroll(0, scrolledDeltaY, 0, unconsumedY, null, 1);
            }
    ...
    static class SavedState extends View.BaseSavedState {
        public static final Parcelable.Creator<SavedState> CREATOR = new Parcelable.Creator<SavedState>() { // from class: im.uwrkaxlmjj.ui.hviews.MyScrollView.SavedState.1
