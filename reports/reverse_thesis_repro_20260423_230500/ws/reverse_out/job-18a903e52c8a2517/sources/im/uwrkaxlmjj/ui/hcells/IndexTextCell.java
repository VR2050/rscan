...
public class IndexTextCell extends FrameLayout {
    ...
    public IndexTextCell(Context context, int left) {
        ...
        BackupImageView backupImageView = (BackupImageView) this.pointView.findViewById(R.attr.iv_head);
        this.iv_head = backupImageView;
        backupImageView.setRoundRadius(AndroidUtilities.dp(5.0f));
        this.view_fc_point = this.pointView.findViewById(R.attr.tv_fc_point);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (LocaleController.isRTL) {
            AndroidUtilities.dp(this.leftPadding);
        }
        if (this.valueTextView.getVisibility() == 0) {
            viewTop = ((height / 2) - this.textView.getTextHeight()) / 2;
        }
        if (LocaleController.isRTL) {
            viewLeft = (getMeasuredWidth() - this.textView.getMeasuredWidth()) - AndroidUtilities.dp(this.imageView.getVisibility() != 0 ? this.leftPadding : 60.0f);
        } else {
            viewLeft = AndroidUtilities.dp(this.imageView.getVisibility() != 0 ? this.leftPadding : 60.0f);
        }
    ...
    public void setTextAndIcon(String text, String imageLeftUrl, int padding, int rightId, boolean divider) {
        this.textView.setText(text);
        this.valueTextView.setText(null);
        GlideUtils.getInstance().load(imageLeftUrl, getContext(), this.imageView, R.id.img_emtpy_error_default, R.id.img_empty_default);
        this.imageView.setVisibility(0);
        this.valueTextView.setVisibility(8);
        this.valueImageView.setVisibility(0);
        this.valueImageView.setImageResource(rightId);
