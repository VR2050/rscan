...
public class MrzRecognizer {
    ...
    */
    private static im.uwrkaxlmjj.messenger.MrzRecognizer.Result recognizeBarcode(android.graphics.Bitmap r13) {
        /*
            Method dump skipped, instruction units count: 612
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: im.uwrkaxlmjj.messenger.MrzRecognizer.recognizeBarcode(android.graphics.Bitmap):im.uwrkaxlmjj.messenger.MrzRecognizer$Result");
    }
        ...
        if (maxLength >= 30 && lineCount >= 2) {
            ...
            for (Rect[] line : charRects) {
                ...
                while (i3 < length) {
                    ...
                    int maxLength3 = (y * 15) + 15;
                    dst.set(x * 10, y * 15, (x * 10) + 10, maxLength3);
            ...
            String[] mrzLines = TextUtils.split(mrz, ShellAdbUtils.COMMAND_LINE_END);
            Result result = new Result();
            if (mrzLines.length >= 2 && mrzLines[0].length() >= 30 && mrzLines[1].length() == mrzLines[0].length()) {
                result.rawMRZ = TextUtils.join(ShellAdbUtils.COMMAND_LINE_END, mrzLines);
                ...
                if (type == 'P') {
                    ...
                    if (mrzLines[0].length() == 44) {
                        ...
                        if (lastNameEnd != -1) {
                            result.lastName = mrzLines[0].substring(5, lastNameEnd).replace(Typography.less, ' ').replace('0', 'O').trim();
                            result.firstName = mrzLines[0].substring(lastNameEnd + 2).replace(Typography.less, ' ').replace('0', 'O').trim();
                        ...
                        String number = mrzLines[1].substring(i, 9).replace(Typography.less, ' ').replace('O', '0').trim();
                    ...
                    if (mrzLines.length == 3 && mrzLines[0].length() == 30 && mrzLines[2].length() == 30) {
                        result.issuingCountry = mrzLines[0].substring(2, 5);
                        String number2 = mrzLines[0].substring(5, 14).replace(Typography.less, ' ').replace('O', '0').trim();
                        ...
                        if (expiryDateChecksum2 == getNumber(mrzLines[1].charAt(14)) || mrzLines[1].charAt(14) == '<') {
                            parseExpiryDate(expiryDate2, result);
                        }
                        result.nationality = mrzLines[1].substring(15, 18);
                        int lastNameEnd2 = mrzLines[2].indexOf("<<");
                        if (lastNameEnd2 != -1) {
                            result.lastName = mrzLines[2].substring(0, lastNameEnd2).replace(Typography.less, ' ').trim();
                            int expiryDateChecksum3 = lastNameEnd2 + 2;
                            result.firstName = mrzLines[2].substring(expiryDateChecksum3).replace(Typography.less, ' ').trim();
                        }
                        ...
                        if ("FRA".equals(result.issuingCountry) && type == 'I' && mrzLines[0].charAt(1) == 'D') {
                            ...
                            result.lastName = mrzLines[0].substring(5, 30).replace(Typography.less, ' ').trim();
                            result.firstName = mrzLines[1].substring(13, 27).replace("<<", ", ").replace(Typography.less, ' ').trim();
                            ...
                            if (lastNameEnd3 == -1) {
                                ...
                                String strSubstring = mrzLines[0].substring(5, lastNameEnd3);
                                c = ' ';
                                c2 = Typography.less;
                                result.lastName = strSubstring.replace(Typography.less, ' ').trim();
                                result.firstName = mrzLines[0].substring(lastNameEnd3 + 2).replace(Typography.less, ' ').trim();
                            }
                            String number4 = mrzLines[1].substring(0, 9).replace(c2, c).replace('O', '0').trim();
