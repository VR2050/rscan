...
public class BallPulseFooter extends InternalAbstract implements RefreshFooter {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        float radius = (fMin - (f * 2.0f)) / 6.0f;
        float x = (width / 2.0f) - ((radius * 2.0f) + f);
        float y = height / 2.0f;
        long now = System.currentTimeMillis();
        int i = 0;
        while (i < 3) {
            long time = (now - this.mStartTime) - ((long) ((i + 1) * 120));
            float percent = this.mInterpolator.getInterpolation(time > 0 ? (time % 750) / 750.0f : 0.0f);
            canvas.save();
            float translateX = x + (radius * 2.0f * i) + (this.mCircleSpacing * i);
