...
public final class TrackSelectionUtil {
    ...
    static int[] getBitratesUsingFutureInfo(MediaChunkIterator[] iterators, Format[] formats, long maxDurationUs, int[] bitrates) {
        ...
        for (int i = 0; i < trackCount; i++) {
            ...
            if (bitrate != -1) {
                ...
                if (formatBitrate != -1) {
                    bitrateRatios[i] = bitrate / formatBitrate;
    ...
    static int[] getBitratesUsingPastInfo(List<? extends MediaChunk> queue, Format[] formats, long maxDurationUs, int[] bitrates) {
        ...
        if (maxDurationUs != 0 && (queueAverageBitrate = getAverageQueueBitrate(queue, maxDurationUs)) != -1 && (queueFormatBitrate = queue.get(queue.size() - 1).trackFormat.bitrate) != -1) {
            float queueBitrateRatio = queueAverageBitrate / queueFormatBitrate;
