...
public final class AdtsReader implements ElementaryStreamReader {
    ...
    public void createTracks(ExtractorOutput extractorOutput, TsPayloadReader.TrackIdGenerator idGenerator) {
        ...
        if (this.exposeId3) {
            ...
            trackOutputTrack.format(Format.createSampleFormat(idGenerator.getFormatId(), MimeTypes.APPLICATION_ID3, null, -1, null));
            return;
        }
    ...
    private boolean checkSyncPositionValid(ParsableByteArray pesBuffer, int syncPositionCandidate) {
        ...
        return isAdtsSyncBytes(pesBuffer.data[nextSyncPosition], pesBuffer.data[nextSyncPosition + 1]) && (this.firstFrameVersion == -1 || ((pesBuffer.data[nextSyncPosition + 1] & 8) >> 3) == currentFrameVersion);
    }

    private boolean isAdtsSyncBytes(byte firstByte, byte secondByte) {
        int syncWord = ((firstByte & UByte.MAX_VALUE) << 8) | (secondByte & UByte.MAX_VALUE);
