...
public final class PesReader implements TsPayloadReader {
                ...
                if (i2 != 1) {
                    if (i2 == 2) {
                        if (continueRead(data, this.pesScratch.data, Math.min(10, this.extendedHeaderLength)) && continueRead(data, null, this.extendedHeaderLength)) {
                            ...
                            padding = this.dataAlignmentIndicator ? 4 : 0;
                            flags |= padding;
    ...
    private boolean parseHeader() {
        ...
        int bits = this.pesScratch.readBits(8);
        this.extendedHeaderLength = bits;
        if (packetLength == 0) {
            this.payloadSize = -1;
        } else {
            this.payloadSize = ((packetLength + 6) - 9) - bits;
        }
        return true;
    }
