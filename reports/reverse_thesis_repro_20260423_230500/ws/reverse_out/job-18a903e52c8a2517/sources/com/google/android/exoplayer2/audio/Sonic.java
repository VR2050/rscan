...
final class Sonic {
    ...
    public void queueInput(ShortBuffer buffer) {
        ...
        int framesToWrite = iRemaining / i;
        int bytesToWrite = i * framesToWrite * 2;
        short[] sArrEnsureSpaceForAdditionalFrames = ensureSpaceForAdditionalFrames(this.inputBuffer, this.inputFrameCount, framesToWrite);
        this.inputBuffer = sArrEnsureSpaceForAdditionalFrames;
        buffer.get(sArrEnsureSpaceForAdditionalFrames, this.inputFrameCount * this.channelCount, bytesToWrite / 2);
    ...
    private void moveNewSamplesToPitchBuffer(int originalOutputFrameCount) {
        ...
        short[] sArrEnsureSpaceForAdditionalFrames = ensureSpaceForAdditionalFrames(this.pitchBuffer, this.pitchFrameCount, frameCount);
        this.pitchBuffer = sArrEnsureSpaceForAdditionalFrames;
        short[] sArr = this.outputBuffer;
        int i = this.channelCount;
        System.arraycopy(sArr, originalOutputFrameCount * i, sArrEnsureSpaceForAdditionalFrames, this.pitchFrameCount * i, i * frameCount);
    ...
    private void adjustRate(float rate, int originalOutputFrameCount) {
        ...
        while (true) {
            ...
            if (position < i3 - 1) {
                ...
                if (i8 == oldSampleRate) {
                    this.oldRatePosition = 0;
                    Assertions.checkState(i2 == newSampleRate);
                    this.newRatePosition = 0;
                }
                position++;
            } else {
    ...
    private int skipPitchPeriod(short[] samples, int position, float speed, int period) {
        ...
        short[] sArrEnsureSpaceForAdditionalFrames = ensureSpaceForAdditionalFrames(this.outputBuffer, this.outputFrameCount, newFrameCount);
        this.outputBuffer = sArrEnsureSpaceForAdditionalFrames;
        overlapAdd(newFrameCount, this.channelCount, sArrEnsureSpaceForAdditionalFrames, this.outputFrameCount, samples, position, samples, position + period);
    ...
    private int insertPitchPeriod(short[] samples, int position, float speed, int period) {
        ...
        short[] sArrEnsureSpaceForAdditionalFrames = ensureSpaceForAdditionalFrames(this.outputBuffer, this.outputFrameCount, period + newFrameCount);
        this.outputBuffer = sArrEnsureSpaceForAdditionalFrames;
        int i = this.channelCount;
        System.arraycopy(samples, position * i, sArrEnsureSpaceForAdditionalFrames, this.outputFrameCount * i, i * period);
