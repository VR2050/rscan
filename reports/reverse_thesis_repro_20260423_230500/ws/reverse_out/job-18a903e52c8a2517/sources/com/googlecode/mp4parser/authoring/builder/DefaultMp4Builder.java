    ...
    public Container build(Movie movie) {
        ...
        for (SampleAuxiliaryInformationOffsetsBox saio : this.sampleAuxiliaryInformationOffsetsBoxes) {
            ...
            while (true) {
                Object current = b2;
                b2 = ((Box) b2).getParent();
                Iterator<Box> it = ((Container) b2).getBoxes().iterator();
                while (true) {
                    if (!it.hasNext()) {
                        mdat = mdat2;
                        break;
        ...
        for (Map.Entry<String, List<GroupEntry>> sg2 : groupEntryFamilies.entrySet()) {
            ...
            while (i < track.getSamples().size()) {
                ...
                if (last == null || last.getGroupDescriptionIndex() != index) {
                    long j2 = 1;
                    SampleToGroupBox.Entry last2 = new SampleToGroupBox.Entry(j2, index);
        ...
        while (i2 < chunkSizes.length) {
            ...
            for (Track current : movie.getTracks()) {
                ...
                while (true) {
                    ...
                    i = i2;
                    if (j2 >= ((long) chunkSizes2[i2]) + firstSampleOfChunk) {
                        break;
                    }
        ...
        private InterleaveChunkMdat(Movie movie, Map<Track, int[]> chunks, long contentSize) {
            ...
            for (int i = 0; i < chunks.values().iterator().next().length; i++) {
                for (Track track : this.tracks) {
                    ...
                    List<Sample> chunk = DefaultMp4Builder.this.track2Sample.get(track).subList(CastUtils.l2i(firstSampleOfChunk), CastUtils.l2i(((long) chunkSizes[i]) + firstSampleOfChunk));
