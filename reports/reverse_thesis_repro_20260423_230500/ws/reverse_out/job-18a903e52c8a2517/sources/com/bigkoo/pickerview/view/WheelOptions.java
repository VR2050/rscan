...
public class WheelOptions<T> {
    ...
    public void setPicker(List<T> options1Items, List<List<T>> options2Items, List<List<List<T>>> options3Items) {
        ...
        if (list2 != null) {
            this.wv_option3.setAdapter(new ArrayWheelAdapter(list2.get(0).get(0)));
        }
    ...
    public int[] getCurrentItems() {
        ...
        if (list2 != null && list2.size() > 0) {
            currentItems[2] = this.wv_option3.getCurrentItem() <= this.mOptions3Items.get(currentItems[0]).get(currentItems[1]).size() - 1 ? this.wv_option3.getCurrentItem() : 0;
        } else {
    ...
    private void itemSelected(int opt1Select, int opt2Select, int opt3Select) {
        ...
        List<List<List<T>>> list2 = this.mOptions3Items;
        if (list2 != null) {
            this.wv_option3.setAdapter(new ArrayWheelAdapter(list2.get(opt1Select).get(opt2Select)));
