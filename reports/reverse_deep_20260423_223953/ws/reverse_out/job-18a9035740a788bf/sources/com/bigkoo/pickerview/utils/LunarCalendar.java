...
public class LunarCalendar {
    ...
    public static final int[] lunarToSolar(int year, int month, int monthDay, boolean isLeapMonth) {
        ...
        int[] iArr = LUNAR_INFO;
        int dayOffset = (iArr[year - 1900] & 31) - 1;
        ...
        int dayOffset2 = dayOffset + monthDay;
        int leapMonth = (LUNAR_INFO[year - 1900] & 15728640) >> 20;
        if (leapMonth != 0 && (month > leapMonth || (month == leapMonth && isLeapMonth))) {
            dayOffset2 = ((524288 >> (month + (-1))) & LUNAR_INFO[year + (-1900)]) == 0 ? dayOffset2 + 29 : dayOffset2 + 30;
        }
        if (dayOffset2 > 366 || (year % 4 != 0 && dayOffset2 > 365)) {
            year++;
            dayOffset2 = year % 4 == 1 ? dayOffset2 - 366 : dayOffset2 - 365;
        }
        ...
        while (true) {
            ...
            if (year % 4 == 0 && i2 == 2 && iPos + 1 == dayOffset2) {
                solarInfo[1] = i2;
                solarInfo[2] = dayOffset2 - 31;
                break;
            }
            if (iPos < dayOffset2) {
                i2++;
                ...
                if (dayOffset2 > iPos2) {
                    solarInfo[2] = dayOffset2 - iPos2;
                } else if (dayOffset2 == iPos2) {
                    if (year % 4 == 0 && i2 == 2) {
    ...
    private static long solarToInt(int y, int m, int d) {
        int y2 = y - (((m + 9) % 12) / 10);
        return (((y2 * 365) + (y2 / 4)) - (y2 / 100)) + (y2 / 400) + (((r0 * 306) + 5) / 10) + (d - 1);
    }
}
