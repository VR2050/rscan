    ...
    public static boolean canScrollVertically(View targetView, int direction) {
        if (Build.VERSION.SDK_INT < 14) {
            if (!(targetView instanceof AbsListView)) {
                return direction > 0 ? targetView.getScrollY() < 0 : targetView.getScrollY() > 0;
            }
            ViewGroup viewGroup = (ViewGroup) targetView;
            ...
            if (direction > 0) {
                if (childCount > 0) {
                    return absListView.getLastVisiblePosition() < childCount + (-1) || viewGroup.getChildAt(childCount + (-1)).getBottom() > targetView.getPaddingBottom();
                }
                return false;
            }
            if (childCount > 0) {
                return absListView.getFirstVisiblePosition() > 0 || viewGroup.getChildAt(0).getTop() < targetView.getPaddingTop();
            }
            return false;
        }
