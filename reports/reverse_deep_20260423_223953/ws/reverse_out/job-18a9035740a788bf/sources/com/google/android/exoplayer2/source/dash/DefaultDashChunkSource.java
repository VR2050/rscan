...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    public DefaultDashChunkSource(LoaderErrorThrower manifestLoaderErrorThrower, DashManifest manifest, int periodIndex, int[] adaptationSetIndices, TrackSelection trackSelection, int trackType, DataSource dataSource, long elapsedRealtimeOffsetMs, int maxSegmentsPerLoad, boolean enableEventMessageTrack, boolean enableCea608Track, PlayerEmsgHandler.PlayerTrackEmsgHandler playerTrackEmsgHandler) {
        ...
        while (i < this.representationHolders.length) {
            ...
            int i2 = i;
            this.representationHolders[i2] = new RepresentationHolder(periodDurationUs, trackType, representation, enableEventMessageTrack, enableCea608Track, playerTrackEmsgHandler);
            i = i2 + 1;
    ...
    public long getAdjustedSeekPositionUs(long positionUs, SeekParameters seekParameters) {
        for (RepresentationHolder representationHolder : this.representationHolders) {
            if (representationHolder.segmentIndex != null) {
                ...
                long secondSyncUs = (firstSyncUs >= positionUs || segmentNum >= ((long) (representationHolder.getSegmentCount() + (-1)))) ? firstSyncUs : representationHolder.getSegmentStartTimeUs(1 + segmentNum);
    ...
    public void getNextChunk(long playbackPositionUs, long loadPositionUs, List<? extends MediaChunk> queue, ChunkHolder out) {
        ...
        if (segmentNum2 > lastAvailableSegmentNum2) {
            ...
            if (!this.missingLastSegment || segmentNum2 < lastAvailableSegmentNum2) {
                ...
                if (periodDurationUs == C.TIME_UNSET) {
                    ...
                    while (maxSegmentCount2 > 1 && representationHolder2.getSegmentStartTimeUs((((long) maxSegmentCount2) + segmentNum2) - 1) >= periodDurationUs) {
                        maxSegmentCount2--;
                    }
                    maxSegmentCount = maxSegmentCount2;
                }
    ...
    public boolean onChunkLoadError(Chunk chunk, boolean cancelable, Exception e, long blacklistDurationMs) {
        ...
        if (!this.manifest.dynamic && (chunk instanceof MediaChunk) && (e instanceof HttpDataSource.InvalidResponseCodeException) && ((HttpDataSource.InvalidResponseCodeException) e).responseCode == 404 && (segmentCount = (representationHolder = this.representationHolders[this.trackSelection.indexOf(chunk.trackFormat)]).getSegmentCount()) != -1 && segmentCount != 0) {
            long lastAvailableSegmentNum = (representationHolder.getFirstSegmentNum() + ((long) segmentCount)) - 1;
    ...
    protected Chunk newMediaChunk(RepresentationHolder representationHolder, DataSource dataSource, int trackType, Format trackFormat, int trackSelectionReason, Object trackSelectionData, long firstSegmentNum, int maxSegmentCount, long seekTimeUs) {
        ...
        for (int i = 1; i < maxSegmentCount; i++) {
            RangedUri nextSegmentUri = representationHolder.getSegmentUrl(firstSegmentNum + ((long) i));
        ...
        long endTimeUs2 = representationHolder.getSegmentEndTimeUs((firstSegmentNum + ((long) segmentCount)) - 1);
        long periodDurationUs = representationHolder.periodDurationUs;
        long clippedEndTimeUs = (periodDurationUs == C.TIME_UNSET || periodDurationUs > endTimeUs2) ? -9223372036854775807L : periodDurationUs;
    ...
    protected static final class RepresentationHolder {
        ...
        RepresentationHolder(long periodDurationUs, int trackType, Representation representation, boolean enableEventMessageTrack, boolean enableCea608Track, TrackOutput playerEmsgTrackOutput) {
            this(periodDurationUs, representation, createExtractorWrapper(trackType, representation, enableEventMessageTrack, enableCea608Track, playerEmsgTrackOutput), 0L, representation.getIndex());
        }
        ...
        public long getLastAvailableSegmentNum(DashManifest manifest, int periodIndex, long nowUnixTimeUs) {
            ...
            return (liveEdgeTimeUs2 + ((long) availableSegmentCount)) - 1;
        }
