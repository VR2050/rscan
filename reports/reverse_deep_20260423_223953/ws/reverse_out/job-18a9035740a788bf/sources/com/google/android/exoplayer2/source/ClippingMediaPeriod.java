        ...
        public int readData(FormatHolder formatHolder, DecoderInputBuffer buffer, boolean requireFormat) {
            ...
            if (result == -5) {
                ...
                if (format.encoderDelay != 0 || format.encoderPadding != 0) {
                    ...
                    int encoderPadding = ClippingMediaPeriod.this.endUs == Long.MIN_VALUE ? format.encoderPadding : 0;
                    formatHolder.format = format.copyWithGaplessInfo(encoderDelay, encoderPadding);
                }
                return -5;
            }
            if (ClippingMediaPeriod.this.endUs != Long.MIN_VALUE && ((result == -4 && buffer.timeUs >= ClippingMediaPeriod.this.endUs) || (result == -3 && ClippingMediaPeriod.this.getBufferedPositionUs() == Long.MIN_VALUE))) {
                buffer.clear();
                buffer.setFlags(4);
                this.sentEos = true;
                return -4;
