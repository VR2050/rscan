...
import im.uwrkaxlmjj.messenger.audioinfo.mp3.MP3Frame;
...
public class MP3Info extends AudioInfo {
    ...
    public MP3Info(InputStream input, long fileLength) throws ID3v2Exception, IOException, MP3Exception {
        this(input, fileLength, Level.FINEST);
    }

    public MP3Info(InputStream input, final long fileLength, Level debugLevel) throws ID3v2Exception, IOException, MP3Exception {
        this.brand = "MP3";
        this.version = "0";
        ...
        if (this.duration <= 0 || this.duration >= 3600000) {
            try {
                this.duration = calculateDuration(data, fileLength, new StopReadCondition() { // from class: im.uwrkaxlmjj.messenger.audioinfo.mp3.MP3Info.1
                    ...
                    @Override // im.uwrkaxlmjj.messenger.audioinfo.mp3.MP3Info.StopReadCondition
        ...
        if (frame != null) {
            ...
            while (true) {
                if (numberOfFrames2 != cbrThreshold || vbr || totalLength <= 0) {
                    ...
                    MP3Frame nextFrame = mP3Info.readNextFrame(mP3Input, stopCondition, frame);
                    frame = nextFrame;
                    if (nextFrame != null) {
