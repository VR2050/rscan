...
public class StatsController extends BaseController {
    ...
    private static final ThreadLocal<Long> lastStatsSaveTime = new ThreadLocal<Long>() { // from class: im.uwrkaxlmjj.messenger.StatsController.1
    ...
    private StatsController(int account) {
        ...
        this.saveRunnable = new Runnable() { // from class: im.uwrkaxlmjj.messenger.StatsController.2
        ...
        if (needConvert) {
            ...
            for (int i = 3; a2 < i; i = 3) {
                this.callsTotalTime[a2] = sharedPreferences.getInt("callsTotalTime" + a2, 0);
                this.resetStatsDate[a2] = sharedPreferences.getLong("resetStatsDate" + a2, 0L);
                for (int b2 = 0; b2 < 7; b2++) {
                    this.sentBytes[a2][b2] = sharedPreferences.getLong("sentBytes" + a2 + "_" + b2, 0L);
                    this.receivedBytes[a2][b2] = sharedPreferences.getLong("receivedBytes" + a2 + "_" + b2, 0L);
                    this.sentItems[a2][b2] = sharedPreferences.getInt("sentItems" + a2 + "_" + b2, 0);
                    this.receivedItems[a2][b2] = sharedPreferences.getInt("receivedItems" + a2 + "_" + b2, 0);
                }
                long[] jArr = this.resetStatsDate;
                if (jArr[a2] == 0) {
                    save2 = true;
                    jArr[a2] = System.currentTimeMillis();
                }
                a2++;
            }
            if (save2) {
