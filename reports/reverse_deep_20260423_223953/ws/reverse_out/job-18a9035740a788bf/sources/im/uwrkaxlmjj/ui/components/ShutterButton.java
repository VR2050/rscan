...
public class ShutterButton extends View {
    ...
    public ShutterButton(Context context) {
        ...
        this.longPressed = new Runnable() { // from class: im.uwrkaxlmjj.ui.components.ShutterButton.1
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (this.state != State.RECORDING) {
            if (this.redProgress != 0.0f) {
                canvas.drawCircle(cx, cy, AndroidUtilities.dp(26.0f) * scale, this.redPaint);
                return;
            }
            return;
        }
        ...
        canvas.drawCircle(cx, cy, AndroidUtilities.dp(26.0f) * scale * this.redProgress, this.redPaint);
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            float dy = (y < 0.0f || y > ((float) getMeasuredHeight())) ? y : 0.0f;
