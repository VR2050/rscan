...
public class PacmanAnimation {
    ...
    private void update() {
        ...
        float f = this.progress + (dt / 400.0f);
        this.progress = f;
        if (f > 1.0f) {
            this.progress = 1.0f;
        }
        float f2 = this.translationProgress + (dt / 2000.0f);
        this.translationProgress = f2;
        if (f2 > 1.0f) {
            this.translationProgress = 1.0f;
        }
        float f3 = this.ghostProgress + (dt / 200.0f);
    ...
    public void draw(Canvas canvas, int cy) {
        ...
        canvas.drawArc(this.rect, rad, 360 - (r10 * 2), true, this.paint);
        this.paint.setColor(-16777216);
