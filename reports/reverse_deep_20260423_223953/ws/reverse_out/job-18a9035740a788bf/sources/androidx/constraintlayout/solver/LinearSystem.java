...
public class LinearSystem {
    ...
    private void displayRows() {
        ...
        String s = "";
        for (int i = 0; i < this.mNumRows; i++) {
            s = (s + this.mRows[i]) + ShellAdbUtils.COMMAND_LINE_END;
        }
        System.out.println(s + this.mGoal + ShellAdbUtils.COMMAND_LINE_END);
    }

    public void displayReadableRows() {
        displaySolverVariables();
        String s = " num vars " + this.mVariablesID + ShellAdbUtils.COMMAND_LINE_END;
        for (int i = 0; i < this.mVariablesID + 1; i++) {
            SolverVariable variable = this.mCache.mIndexedVariables[i];
            if (variable != null && variable.isFinalValue) {
                s = s + " $[" + i + "] => " + variable + " = " + variable.computedValue + ShellAdbUtils.COMMAND_LINE_END;
            }
        }
        String s2 = s + ShellAdbUtils.COMMAND_LINE_END;
        for (int i2 = 0; i2 < this.mVariablesID + 1; i2++) {
            ...
            if (variable2 != null && variable2.isSynonym) {
                ...
                s2 = s2 + " ~[" + i2 + "] => " + variable2 + " = " + synonym + " + " + variable2.synonymDelta + ShellAdbUtils.COMMAND_LINE_END;
            }
        }
        String s3 = s2 + "\n\n #  ";
        ...
        if (this.mGoal != null) {
            s3 = s3 + "Goal: " + this.mGoal + ShellAdbUtils.COMMAND_LINE_END;
        }
    ...
    public void displayVariablesReadableRows() {
        ...
        for (int i = 0; i < this.mNumRows; i++) {
            if (this.mRows[i].variable.mType == SolverVariable.Type.UNRESTRICTED) {
                s = (s + this.mRows[i].toReadableString()) + ShellAdbUtils.COMMAND_LINE_END;
            }
        }
        System.out.println(s + this.mGoal + ShellAdbUtils.COMMAND_LINE_END);
    }
